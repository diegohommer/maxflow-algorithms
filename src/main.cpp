...
int benchmark_mode(Algorithm algo, const char* input_path, const char* output_name) {
    ...
    // Sort the graphs by the number of vertices first, then by the number of arcs if vertices are equal
    ...
    for (auto& graph : graphs) {
        ...
        for (int i = 0; i < num_runs; ++i) {
            ...
            FordResult result = ford_fulkerson(copy_graph, copy_graph.get_source(), copy_graph.get_sink(), algo, true);
