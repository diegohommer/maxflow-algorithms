...
int32_t Range[] = {1000000, 500000, 250000, 125000, 62500, 31250, 15625, 7812, 3906, 1953,
                   976,     488,    244,    122,    61,    31,    15,    7,    4,    2};

main(argc, argv) int argc;
char *argv[];
{
    Graph *G, *Mesh(), *RLevel(), *R2Level(), *Match(), *SquareMesh(), *BasicLine(),
        *ExponentialLine(), *DExponentialLine(), *DinicBadCase(), *GoldBadCase(), *Cheryian();
    ...
    if (argc < 2)
        Barf("Usage: makegraph instance_type [specific parameters]");
    fct = atoi(argv[1]);
    ...
    switch (fct) {
        case 1:
            ...
            fprintf(f,
                    "c %" PRId32 " Rows, %" PRId32 " columns, capacities in range [0, %" PRId32
                    "]\n",
                    dim1, dim2, range);
        ...
        case 4:
            ...
            fprintf(f,
                    "c %" PRId32 " vertices, %" PRId32 " degree, capacities in range [0, %" PRId32
                    "]\n",
                    dim1, dim2, range);
        ...
        case 5:
            ...
            fprintf(f,
                    "c %" PRId32 " x %" PRId32 " vertices, %" PRId32 " degree, range [0,%" PRId32
                    "]\n",
        ...
        case 7:
            ...
            fprintf(f,
                    "c %" PRId32 " x %" PRId32 " vertices, degree %" PRId32 ", range [0,%" PRId32
                    "]\n",
                    dim1, dim2, deg, range);
        ...
        case 11:
            ...
            fprintf(f,
                    "c n = %" PRId32 ", m = %" PRId32 ", c = %" PRId32 ", total vertices %" PRId32
                    " \n",
...
Graph *Match(int32_t n, int32_t d, int32_t r)
{
    ...
    for (j = 1; j <= n; j++) {
        ...
        for (i = 0; i < d; i++) AddEdge(j, n + x[i], RandomInteger(1, r), G);
    }
...
Gadget(a, b, n, m, c, G, very_big) int32_t a, b, n, m, c;
Graph *G;
int32_t very_big;
{
    int32_t i, j, v, w;
...
AddEdge(v1, v2, a, G) int32_t v1, v2, a;
Graph *G;
{
...
StrAppend(s1, s2, s3) char *s1, *s2, *s3;
{
    while (*s1) *s3++ = *s1++;
    while (*s2) *s3++ = *s2++;
    *s3 = 0;
...
Queue *Q;
{
    return (Q->tail >= Q->head) ? Q->tail - Q->head : Q->tail - Q->head + Q->size;
}

QueueEmpty(Q) Queue *Q;
{
...
*/
RandomPermutation(perm, n) int32_t perm[], n;
{
    int32_t i, j, t;
...
RandPerm(perm, n) int32_t perm[], n;
{
    int32_t i, j, t;
...
be improved */
RandomSubset(low, high, n, x) int32_t low, high, n, *x;
{
    int32_t i, j, r, flag;
